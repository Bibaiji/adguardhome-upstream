...
if [[ $IPv4 == "true" ]]; then
	if [[ $IPv6 == "true" ]]; then
		...
		curl -o "/var/tmp/default.upstream" https://raw.kgithub.com/Bibaiji/adguardhome-upstream/master/v6.conf > /dev/null 2>&1
	else
		echo "$DATE: IPv4 connection available."
		curl -o "/var/tmp/default.upstream" https://raw.kgithub.com/Bibaiji/adguardhome-upstream/master/v4.conf > /dev/null 2>&1
	fi
else
	if [[ $IPv6 == "true" ]]; then
		echo "$DATE: IPv6 connection available."
		curl -o "/var/tmp/default.upstream" https://raw.kgithub.com/Bibaiji/adguardhome-upstream/master/v6only.conf > /dev/null 2>&1
	else
...
wget https://raw.kgithub.com/Bibaiji/Chinese-list/master/CHN.ALL.agh -O/var/tmp/2.upstream
